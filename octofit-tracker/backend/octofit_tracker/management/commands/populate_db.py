...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        flying = Workout.objects.create(name='Flying', description='Aerial', difficulty='hard')
        ...
        Activity.objects.create(user=bruce, type='flying', duration=60, date=timezone.now().date())
        Activity.objects.create(user=clark, type='pushups', duration=20, date=timezone.now().date())
